...
contract SwapOracle is OwnableUpgradeable, ISwapOracle {
    ...
    function initialize(address wnc_) public initializer {
        __Ownable_init();
        wnc = wnc_;
    }

    function updateWNC(address val) public onlyOwner {
        wnc = val;
    }
    ...
    function queryInstantAmountOut(
        ...
        for (uint256 i; i < paths.length - 1 && tokenAmountOut > 0; i++) {
            address token0 = paths[i];
            address token1 = paths[i + 1];
            tokenAmountOut = _queryInstantAmountOut(
                token0,
                token1,
                tokenAmountOut
            );
    ...
    function queryTokenPrices(address[] memory tokens)
        public
        view
        override
    ...
    function queryPairPrices(address[] memory pairs)
        public
        view
        override
        returns (uint256[] memory prices)
    ...
    function queryTokenPrice(address token)
        public
        view
        override
        returns (uint256)
    {
        if (token == address(0)) {
            return queryInstantAmountOut(wnc, dominationToken, UNIT);
    ...
    function queryPairPrice(address pairAddress)
        public
        view
        override
    ...
    function _queryInstantAmountOut(
        ...
        if (pair != address(0)) {
            (uint112 reserve0, uint112 reserve1, ) = ISwapPair(pair)
                .getReserves();
