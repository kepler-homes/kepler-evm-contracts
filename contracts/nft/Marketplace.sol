    ...
    IMarketplace
{
    ...
    function initialize() public initializer {
        ...
        _domainSeparator = keccak256(
            abi.encode(DOMAIN_TYPE_HASH, keccak256(bytes("KeplerMarketplace")), _getChainId(), address(this))
        );
    }
    ...
    function _acceptOffer(uint256 offerId) private {
        ...
        if (offer.nftType == 1155) {
            ...
            transferNFT(offer.nft, sender, offer.offerer, offer.tokenId);
        }
    ...
    function transfer(TransferRequest memory request) public override {
        if (request.nftType == 721) {
            transferNFT(request.nft, msg.sender, request.to, request.tokenId);
        } else {
    ...
    function _buy(ListRequest memory req, uint256 buyAmount) private {
        ...
        if (req.nftType == 1155) {
            ...
            transferNFT(req.nft, req.seller, buyer, req.tokenId);
        }
